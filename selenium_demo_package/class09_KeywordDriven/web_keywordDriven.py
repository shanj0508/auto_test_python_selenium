...
from selenium_demo_package.class09_KeywordDriven.chrome_options import Options
...
class WebKey:
    ...
    # # 断言
    # def assert_(self, text, expectText):
    #     assert text == expectText, '断言失败'
    ...
    # 清除文本框内容并输入新值
    def clear_input(self, name, value, newValue):
        el = self.locator(name, value)
        el.clear()
        el.send_keys(newValue)

    # 获取属性值
    def get_attribute(self, name, value, attributeName):
        return self.locator(name, value).get_attribute(attributeName)
